...
from uldk_parcel import (
    getParcelById, getParcelById2, getRegionById,
    DEFAULT_SRID, PARCEL_BY_ID_OR_NR, REQ_REGION_BY_ID, RES_REGION_META,
)
from data.dane import PARCEL_BY_ID, PARCEL_BY_ID2, REGION_BY_ID
    ...
    def test_getParcelById(self, mock_cls):
        for case in PARCEL_BY_ID:
            with self.subTest(name=case["name"]):
                ...
                out = getParcelById(case["name"], case["object_type"])
                self.assertEqual(out, case["expected"])
                mock_cls.assert_called_once_with(
                    {"request": PARCEL_BY_ID_OR_NR["request"], "id": case["name"],
                     "result": ",".join(PARCEL_BY_ID_OR_NR["result"]), "srid": DEFAULT_SRID},
                    case["object_type"])

    @patch('uldk_parcel.RS')
    def test_getParcelById2(self, mock_cls):
        for case in PARCEL_BY_ID2:
            with self.subTest(name=case["name"]):
                mock_cls.reset_mock()
                mock_cls.return_value = Mock(data=case["expected"])
                out = getParcelById2(case["name"])
                self.assertEqual(out, case["expected"])
                mock_cls.assert_called_once_with(
                    {"request": PARCEL_BY_ID_OR_NR["request"], "id": case["name"],
                     "result": ",".join(PARCEL_BY_ID_OR_NR["result"]), "srid": DEFAULT_SRID})
            ...
            with self.subTest(id=case["id"]):
                ...
                mock_cls.assert_called_once_with(
                    {"request": REQ_REGION_BY_ID, "id": case["id"],
                     "result": ",".join(RES_REGION_META), "srid": DEFAULT_SRID})
